...
var spawned = false
var spawn_positions = []  # Array to store possible spawn positions
...
func _ready():
	...
	# Fill the spawn_positions array
	var tilemap = Global.Tilemap_Floor
	var used_cells = tilemap.get_used_cells()
	for cell in used_cells:
		if tilemap.get_cellv(cell) == 0:
			spawn_positions.append(tilemap.map_to_world(cell) * tilemap.scale.x + (tilemap.cell_size * tilemap.scale.x / 2))
...
func get_random_spawn_position():
	# Pick a random position from the spawn_positions array
